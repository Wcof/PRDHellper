...
def test_sync_from_code_creates_traceability(tmp_path: Path):
    ...
    page_prd = tmp_path / "docs/prd/pages/demo.md"
    ...
    route_text = (tmp_path / "docs/prd/01-页面路由清单.md").read_text(encoding="utf-8")
    assert "page_id" in route_text
    assert "`/demo`" in route_text

    index_path = tmp_path / "docs/prd/.index/traceability.json"
...
def test_sync_from_prd_backfills_feature_and_change(tmp_path: Path):
    ...
    page = tmp_path / "docs/prd/pages/custom.md"
    page.write_text(
        """---
page_id: page-custom
route: /custom
...
""",
    ...
    feature_text = (tmp_path / "docs/prd/02-功能清单.md").read_text(encoding="utf-8")
    assert "feat-custom-core" in feature_text
    assert "page-custom" in feature_text

    change_file = tmp_path / "docs/prd/changelog/custom-change.md"
...
def test_audit_strict_reports_unlinked_feature(tmp_path: Path):
    run_cli("init-project", str(tmp_path), "--mode", "greenfield")
    (tmp_path / "docs/prd/pages/demo.md").write_text(
        """---
page_id: page-demo
route: /demo
code_paths: ["src/pages/demo.tsx"]
...
""",
    ...
    (tmp_path / "docs/prd/changelog/demo-change.md").write_text(
        """# 页面变更记录
...
def test_collectors_ignore_placeholder_rows(tmp_path: Path):
    run_cli("init-project", str(tmp_path), "--mode", "greenfield")
    feature = tmp_path / "docs/prd/02-功能清单.md"
    feature.write_text(
        """# 功能清单
...
""",
    ...
    route = tmp_path / "docs/prd/01-页面路由清单.md"
    route.write_text(
        """# 页面路由清单
...
| page-real | m | n | `/real` | `src/real.ts` | `docs/prd/pages/real.md` | done |
""",
        encoding="utf-8",
    )
    ...
    routes = mod.collect_routes(tmp_path, "docs/prd")
...
def test_scan_axure_skips_auxiliary_export_pages(tmp_path: Path):
    ...
    report = (tmp_path / "docs/prd/imports/axure-pages.md").read_text(encoding="utf-8")
    ...
    pages_dir = tmp_path / "docs/prd/pages"
...
def test_sync_from_prd_repairs_missing_frontmatter_fields(tmp_path: Path):
    run_cli("init-project", str(tmp_path), "--mode", "greenfield")
    page = tmp_path / "docs/prd/pages/minimal.md"
    page.write_text(
        """---
route: /minimal
---
...
def test_diff_sync_treats_component_changes_as_code_changes(tmp_path: Path):
    ...
    report = tmp_path / f"docs/prd/audit/{dt.date.today().isoformat()}-diff-sync.md"
