...
3. 用一句话向用户呈现推断结果，作为默认假设并继续生成：

> 根据你的描述，这是一个【{商业属性} × {功能类型}】产品{，简要理由}。我先按该假设生成完整 PRD 草案；如有不对你可直接纠正，我再回改对应章节。

4. 不得因为信息不足而停在“先提问后产出”；缺失信息统一写入 `[TODO: ...]`，并继续完成全文草案。
5. 直接加载定型参考文件以确定各章节的适配规则：
...
- 如果用户提供的上下文非常有限，仍需先输出完整结构化草案；将补充信息清单放在文末“待确认项”，不阻塞当前交付。
...
| 页面代码 | [TODO] |
| 页面 PRD | [TODO] |
| 功能清单 | [TODO: 当前项目 PRD 根目录]/02-功能清单.md |
...
2. 修改页面代码。
3. 判断本次修改是否影响 PRD。
4. 同步更新 docs/prd/pages/{route-slug}.md。
5. 同步更新 docs/prd/02-功能清单.md。
6. 同步更新 docs/prd/changelog/{route-slug}-change.md。
7. 输出代码与 PRD 一致性检查。
